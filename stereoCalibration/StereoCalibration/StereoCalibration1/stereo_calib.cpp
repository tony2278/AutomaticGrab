...
// OR ELSE HARTLEY'S METHOD
 ...
 // from the fundamental matrix
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            Mat img = imread("calib_images/" + goodImageList[i*2+k], 0), rimg, cimg;
...
int main(int argc, char** argv)
{
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|9|}{h|6|}{s|1.0|}{nr||}{help||}{@input|calib_images/stereo_calib.xml|}");
    if (parser.has("help"))
        return print_help();
    ...
    StereoCalib(imagelist, boardSize, squareSize, true/*displayCorners*/, true, showRectified);
    return 0;
}
